...
import { BarChart3, Users, Database, Loader2, Menu, X } from 'lucide-react'
...
function PipelineIndicator({ status, progress, compact }) {
  ...
  return (
    <div className="hidden sm:flex items-center gap-2.5 px-3 py-1.5 2xl:px-3.5 2xl:py-2 bg-white/10 rounded-lg">
      <Loader2 className="w-3.5 h-3.5 2xl:w-4 2xl:h-4 animate-spin text-steel-300" aria-hidden />
      <span className="text-xs 2xl:text-sm font-medium text-steel-200">
        {label}... {pctText}
      </span>
      <div className="w-16 2xl:w-24 h-1.5 bg-white/10 rounded-full overflow-hidden">
        <div
          className={`h-full rounded-full transition-all duration-500 ${progress < 0 ? 'bg-steel-300 animate-pulse w-full' : 'bg-steel-300'}`}
...
export default function Layout() {
  ...
  return (
    <div className="h-dvh min-h-0 flex flex-col">
          ...
          <div className="flex items-center gap-4 lg:gap-6 2xl:gap-8">
            ...
            <div className="flex items-center gap-2.5 2xl:gap-3">
              <div className="w-8 h-8 sm:w-9 sm:h-9 2xl:w-10 2xl:h-10 rounded-lg bg-white/10 flex items-center justify-center">
                <Database className="w-4 h-4 sm:w-5 sm:h-5 2xl:w-5 2xl:h-5 text-steel-200"/>
              </div>
      ...
      <main className="flex-1 min-h-0 overflow-hidden bg-[#f0f3f8]">
        <Outlet />
      </main>
    </div>
  )
